...
--========================================================--
-- TAG PLAYER BOARD (6 PLAYERS + MONEY SORT)
--========================================================--

local TAGButton = Instance.new("TextButton")
TAGButton.Name = "TAG_BUTTON"
TAGButton.Size = UDim2.new(0, 34, 0, 40)
TAGButton.Position = UDim2.new(0, 98, 0.43, 0)
TAGButton.BackgroundColor3 = Color3.fromRGB(18,18,18)
TAGButton.BackgroundTransparency = 0.15
TAGButton.Text = "TAG"
TAGButton.TextSize = 9
TAGButton.Font = INFO_FONT
...
local TAGPanel = Instance.new("Frame")
TAGPanel.Name = "TAG_PLAYER_LIST"
TAGPanel.Size = UDim2.new(0,138,0,0)
-- Danh sách TAG nằm ngay phía trên FPS/NP/DP, có khe hở nhỏ.
TAGPanel.Position = UDim2.new(0,2,0.43,-28)
TAGPanel.AnchorPoint = Vector2.new(0,1)
TAGPanel.BackgroundColor3 = Color3.fromRGB(15,15,15)
TAGPanel.BackgroundTransparency = 0.08
TAGPanel.BorderSizePixel = 0
TAGPanel.Visible = false
TAGPanel.ZIndex = 20
TAGPanel.Parent = ScreenGui

local TAGPadding = Instance.new("UIPadding")
TAGPadding.PaddingTop = UDim.new(0,4)
TAGPadding.PaddingBottom = UDim.new(0,4)
TAGPadding.PaddingLeft = UDim.new(0,5)
TAGPadding.PaddingRight = UDim.new(0,5)
TAGPadding.Parent = TAGPanel

local TAGLayout = Instance.new("UIListLayout")
TAGLayout.Padding = UDim.new(0,1)
...
TAGLayout.Parent = TAGPanel

local TAGCorner2 = Instance.new("UICorner")
TAGCorner2.CornerRadius = UDim.new(0,8)
TAGCorner2.Parent = TAGPanel

local TAGOpen = false
local function ShortMoney(v)
	v = tonumber(v) or 0
	-- Dưới 1 triệu giữ nguyên số; từ 1 triệu dùng M viết hoa.
	if v >= 1000000000 then
		local n = math.floor((v / 1000000000) * 10) / 10
		return (n % 1 == 0 and string.format("%.0fB", n) or string.format("%.1fB", n))
	elseif v >= 1000000 then
		local n = math.floor((v / 1000000) * 10) / 10
		return (n % 1 == 0 and string.format("%.0fM", n) or string.format("%.1fM", n))
	end
...
local function GetMoney(plr)
	...
	if ls then
		for _,x in ipairs(ls:GetChildren()) do
			if x:IsA("IntValue") or x:IsA("NumberValue") then
				if string.find(string.lower(x.Name),"money") or string.find(string.lower(x.Name),"cash") or string.find(string.lower(x.Name),"coin") or string.find(string.lower(x.Name),"dollar") then
					return x.Value
				end
			end
		end
...
local function UpdateTAG()
	for _,v in ipairs(TAGPanel:GetChildren()) do
		if v:IsA("TextLabel") then v:Destroy() end
	end
	local list = Players:GetPlayers()
	table.sort(list,function(a,b) return GetMoney(a)>GetMoney(b) end)
	for i=1,math.min(6,#list) do
		local p=list[i]
		local row=Instance.new("TextLabel")
		row.Size=UDim2.new(1,0,0,17)
		row.BackgroundColor3=Color3.fromRGB(35,35,35)
		row.Text=p.DisplayName.."  "..ShortMoney(GetMoney(p))
		row.TextSize=9
		row.Font=INFO_FONT
	...
	local count = math.min(6,#list)
	TAGPanel.Size=UDim2.new(0,138,0,count*17 + math.max(0,count-1)*1 + 8)
end

--========================================================--
-- FPS BOOST BUTTON (ngay bên phải TAG)
--========================================================--
local FPSBoostButton = Instance.new("TextButton")
FPSBoostButton.Name = "FPSBoostButton"
FPSBoostButton.Parent = TAGButton.Parent
FPSBoostButton.Size = TAGButton.Size
FPSBoostButton.Position = UDim2.new(
	TAGButton.Position.X.Scale,
	TAGButton.Position.X.Offset + 38,
	TAGButton.Position.Y.Scale,
	TAGButton.Position.Y.Offset
)
FPSBoostButton.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
FPSBoostButton.BackgroundTransparency = 0.15
FPSBoostButton.BorderSizePixel = 0
FPSBoostButton.Text = "FPS"
FPSBoostButton.TextSize = TAGButton.TextSize
FPSBoostButton.TextColor3 = Color3.new(1, 1, 1)
FPSBoostButton.Font = INFO_FONT
FPSBoostButton.ZIndex = TAGButton.ZIndex + 1
...
--========================================================--
-- ⚡ NÚT KÉO RIÊNG CẠNH FPS
-- Chỉ chính nút ⚡ di chuyển, không kéo FPS/TAG/HUB.
-- Có giới hạn trong màn hình để không bị kéo mất.
--========================================================--
local LightningDragButton = Instance.new("TextButton")
LightningDragButton.Name = "LightningDragButton"
LightningDragButton.Size = UDim2.new(0, 34, 0, 40)
local InitialViewport = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(800, 600)
LightningDragButton.Position = UDim2.fromOffset(174, math.floor(InitialViewport.Y * 0.43))
LightningDragButton.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
LightningDragButton.BackgroundTransparency = 0.15
LightningDragButton.BorderSizePixel = 0
LightningDragButton.Text = "⚡"
LightningDragButton.TextSize = 18
LightningDragButton.Font = INFO_FONT
LightningDragButton.TextColor3 = Color3.fromRGB(255, 255, 255)
LightningDragButton.AutoButtonColor = true
LightningDragButton.Active = true
LightningDragButton.ZIndex = FPSBoostButton.ZIndex + 1
LightningDragButton.Parent = ScreenGui

local LightningCorner = Instance.new("UICorner")
LightningCorner.CornerRadius = UDim.new(0, 8)
LightningCorner.Parent = LightningDragButton

-- Kéo bằng đúng ngón tay/click đang chạm vào ⚡.
-- Không dùng InputBegan của ScreenGui nên không ảnh hưởng các nút khác.
local LightningDragging = false
local LightningDragInput = nil
local LightningDragStart = nil
local LightningStartPosition = nil

local function ClampLightningPosition(pos)
	...
	local size = LightningDragButton.AbsoluteSize
	local minX = 0
	local minY = 0
	...
	return UDim2.fromOffset(
		math.clamp(pos.X.Offset, minX, maxX),
		math.clamp(pos.Y.Offset, minY, maxY)
	)
end

LightningDragButton.InputBegan:Connect(function(input)
	if input.UserInputType ~= Enum.UserInputType.MouseButton1
		and input.UserInputType ~= Enum.UserInputType.Touch then
		return
	end

	LightningDragging = true
	LightningDragInput = input
	LightningDragStart = input.Position
	LightningStartPosition = LightningDragButton.Position

	input.Changed:Connect(function()
		if input.UserInputState == Enum.UserInputState.End then
			LightningDragging = false
			LightningDragInput = nil
		end
	end)
end)

UIS.InputChanged:Connect(function(input)
	if not LightningDragging then
		return
	end
	...
	local delta = input.Position - LightningDragStart
	local newPos = UDim2.fromOffset(
		LightningStartPosition.X.Offset + delta.X,
		LightningStartPosition.Y.Offset + delta.Y
	)

	LightningDragButton.Position = ClampLightningPosition(newPos)
end)

-- Giữ nút luôn nằm trong màn hình khi đổi kích thước/ xoay màn hình.
workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
	if LightningDragButton and LightningDragButton.Parent then
		LightningDragButton.Position = ClampLightningPosition(LightningDragButton.Position)
	end
end)
...
local function ApplyFPSBoostToObject(obj)
	...
	pcall(function()
		if obj:IsA("ParticleEmitter")
			...
			if FPSBoostSaved[obj] == nil then
				FPSBoostSaved[obj] = obj.Enabled
			end
			obj.Enabled = false
		...
		elseif obj:IsA("BasePart") then
			if obj.Material ~= Enum.Material.Plastic then
				if FPSBoostSaved[obj] == nil then
					FPSBoostSaved[obj] = obj.Material
				end
...
local function StartFPSBoostNewObjectWatcher()
	...
	FPSBoostDescendantConnection = workspace.DescendantAdded:Connect(function(obj)
		...
		task.defer(function()
			if not FPSBoostEnabled or not obj or not obj.Parent then
				return
			end
			...
			for _, child in ipairs(obj:GetDescendants()) do
				ApplyFPSBoostToObject(child)
			end
		end)
	end)
end

local function StopFPSBoostNewObjectWatcher()
	if FPSBoostDescendantConnection then
		FPSBoostDescendantConnection:Disconnect()
...
local function ApplyFPSBoost()
	...
	-- Tắt hiệu ứng/chi tiết nặng nhất có thể mà không xoá map/UI.
	pcall(function()
	...
	-- Theo dõi object mới được xây/tạo trong lúc FPS Boost đang bật.
	StartFPSBoostNewObjectWatcher()

	-- Khi bật FPS Boost: lưu cap FPS hiện tại rồi lock mục tiêu ở 120.
	pcall(function()
		if type(getfpscap) == "function" then
			FPSCapSaved = getfpscap()
		end
...
local function DisableFPSBoost()
	StopFPSBoostNewObjectWatcher()

	for obj, value in pairs(FPSBoostSaved) do
	...
	-- Tắt FPS Boost: trả cap FPS về đúng mức trước khi bật, nếu môi trường hỗ trợ.
	pcall(function()
		if FPSCapSaved ~= nil and type(setfpscap) == "function" then
			setfpscap(FPSCapSaved)
		end
	end)
	FPSCapSaved = nil
	FPSBoostButton.Text = "FPS"
...
TAGButton.MouseButton1Click:Connect(function()
	TAGOpen=not TAGOpen
	TAGPanel.Visible=TAGOpen
	if TAGOpen then UpdateTAG() end
end)

-- Khi TAG đang mở, click/tap bất kỳ chỗ nào khác nút TAG sẽ đóng danh sách.
...
UIS.InputBegan:Connect(function(Input, GameProcessed)
	...
	for _,Object in ipairs(Objects) do
		if Object == TAGButton or Object:IsDescendantOf(TAGButton) then
			return
		end
	end
...
Players.PlayerAdded:Connect(UpdateTAG)
Players.PlayerRemoving:Connect(UpdateTAG)
...
GodButton.Size = UDim2.new(0, 70, 0, 30)
GodButton.Position = UDim2.new(1, -82, 0.5, -15)
GodButton.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
GodButton.BackgroundTransparency = 0
GodButton.BorderSizePixel = 0
GodButton.Text = "GOD OFF"
GodButton.TextSize = 12
GodButton.TextColor3 = Color3.fromRGB(255,255,255)
GodButton.Font = GAME_FONT
GodButton.Parent = TitleBar
StyleText(GodButton)
...
GodButton.MouseButton1Click:Connect(function()
    ...
    GodButton.Text = GodModeEnabled and "GOD ON" or "GOD OFF"
    ApplyGodMode()
end)
...
local function StartFlyEffect()
    ...
    if not Root then
        -- Giữ trạng thái ON; CharacterAdded sẽ tạo lại khi nhân vật load xong.
        return
    end
    ...
    for _,offset in ipairs({
        ...
        spark.Color = ColorSequence.new(Color3.fromRGB(0, 170, 255))
    ...
    task.spawn(function()
        ...
        while FlyEffectRunning and token == FlyEffectToken do
            local c = Color3.fromRGB(255, 70, 190)
            h.OutlineColor = c
            ...
            for _,flame in ipairs(fireParts) do
                flame.Color = c
                flame.SecondaryColor = Color3.fromRGB(255, 120, 215)
...
-- Nút 💀 nằm chính giữa thanh và kéo đúng từ đầu thanh tới cuối thanh.
local function UpdateSpeedSlider()
    ...
    local centerX = x * lineWidth
    SpeedKnob.Position = UDim2.new(0, centerX - knobWidth / 2, 0, (SpeedLine.Position.Y.Offset + SpeedLine.Size.Y.Offset / 2) - knobWidth / 2)
    SpeedBox.Text = tostring(SpeedRunValue)
end

SpeedKnob.Position = UDim2.new(0, -SpeedKnob.Size.X.Offset / 2, 0, 0)
UpdateSpeedSlider()
...
SpeedSlider.InputBegan:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1
    or i.UserInputType == Enum.UserInputType.Touch then
        -- chỉ bắt khi chạm đúng vùng SpeedRun, không chiếm touch của analog
        draggingSpeed = true
        sliderInput = i
...
UIS.InputChanged:Connect(function(i)
    if not draggingSpeed or i ~= sliderInput then
        return
    end
    ...
    or i.UserInputType == Enum.UserInputType.MouseMovement then
        ...
        local centerX = math.clamp(
            i.Position.X - lineLeft,
            0,
            lineWidth
        )

        local x = centerX / math.max(1, lineWidth)

        SpeedKnob.Position = UDim2.new(
            0,
            centerX - knobWidth / 2,
            0,
            (SpeedLine.Position.Y.Offset + SpeedLine.Size.Y.Offset / 2) - knobWidth / 2
        )
...
AllButton.MouseButton1Click:Connect(function()
	...
	if AllEnabled then
		...
		-- Tắt GODMODE + NOCLIP khi ALL OFF
...
CreateServerButton("🔄 RESET SERVER", function()
    -- Reset: quay lại đúng server đang chơi, không tìm server mới
...
CreateServerButton("🌐 NEW SERVER", function()
    -- New server: ưu tiên server ít người (tối đa 4 người), tránh server hiện tại
...
local EffectEnergySound = SoundService:FindFirstChild("DUMAVNG_EffectEnergySound")
if EffectEnergySound then
    EffectEnergySound:Destroy()
end

EffectEnergySound = Instance.new("Sound")
EffectEnergySound.Name = "DUMAVNG_EffectEnergySound"
EffectEnergySound.SoundId = "rbxassetid://9116279942"
EffectEnergySound.Volume = 1
EffectEnergySound.PlaybackSpeed = 1
EffectEnergySound.Looped = false
EffectEnergySound.Parent = SoundService

local function PlayEffectEnergySound()
    if not EffectEnergySound or not EffectEnergySound.Parent then
        return
    end

    EffectEnergySound:Stop()
    EffectEnergySound.TimePosition = 0
    EffectEnergySound:Play()
end
...
Player.CharacterAdded:Connect(function(Character)
    if not FlyEffectRunning then
        return
    end

    -- Chờ nhân vật mới load hoàn toàn, đặc biệt là HumanoidRootPart.
    local Root = Character:WaitForChild("HumanoidRootPart", 8)
    if not Root or not FlyEffectRunning then
        return
    end

    task.wait(0.15)

    -- StartFlyEffect() sẽ xoá effect của character cũ (nếu còn)
    -- rồi tạo lại lửa + outline trên character mới.
    pcall(StartFlyEffect)

    if EffectButton and EffectButton.Parent then
        EffectButton.Text = "🔥 EFFECT ON"
    end
    if TogglePanelButton and TogglePanelButton.Parent then
        TogglePanelButton.Text = "🔥"
    end
end)
...
-- SPEED RUN INTERACTION AREA
-- Bắt đầu từ mép trên thanh kéo, rộng đúng bằng thanh và kéo xuống dưới.
local SpeedClickArea = Instance.new("TextButton")
SpeedClickArea.Name = "SpeedRunClickArea"
SpeedClickArea.BackgroundTransparency = 1
SpeedClickArea.BorderSizePixel = 0
SpeedClickArea.Text = ""
SpeedClickArea.AutoButtonColor = false
SpeedClickArea.Position = UDim2.new(0, -12, 0, 0)
-- Để vùng tương tác phủ từ mép trên thanh tới đáy panel.
task.defer(function()
    local panel = SpeedLine.Parent
    local top = SpeedLine.AbsolutePosition.Y - panel.AbsolutePosition.Y
    local bottom = panel.AbsoluteSize.Y
    SpeedClickArea.Size = UDim2.new(1, 24, 0, math.max(0, bottom - top))
end)
SpeedClickArea.ZIndex = SpeedLine.ZIndex + 1
SpeedClickArea.Parent = SpeedLine.Parent

local SpeedAreaDragging = false
local SpeedDragInput = nil

local function SetSpeedFromPointerX(x)
    local left = SpeedLine.AbsolutePosition.X
    local width = SpeedLine.AbsoluteSize.X
    local ratio = math.clamp((x - left) / math.max(width, 1), 0, 1)
    SpeedRunValue = math.floor(16 + ratio * (300 - 16) + 0.5)
    if UpdateSpeedSlider then
        UpdateSpeedSlider()
    end
    ApplySpeed()
end

SpeedClickArea.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1
        or input.UserInputType == Enum.UserInputType.Touch then
        SpeedAreaDragging = true
        SpeedDragInput = input
        SetSpeedFromPointerX(input.Position.X)
    end
end)

SpeedClickArea.InputEnded:Connect(function(input)
    if input == SpeedDragInput then
        SpeedAreaDragging = false
        SpeedDragInput = nil
    end
end)

game:GetService("UserInputService").InputChanged:Connect(function(input)
    if not SpeedAreaDragging then
        return
    end

    if SpeedDragInput and SpeedDragInput.UserInputType == Enum.UserInputType.Touch then
        if input.UserInputType == Enum.UserInputType.Touch
            and input == SpeedDragInput then
            SetSpeedFromPointerX(input.Position.X)
        end
...
-- Giữ WalkSpeed ổn định khi dùng analog/mobile joystick, tránh bị script khác
-- đổi qua lại làm nhân vật giật tới lui.
RunService.RenderStepped:Connect(function()
    if SpeedRunEnabled then
        local Character = Player.Character
        local Humanoid = Character and Character:FindFirstChildOfClass("Humanoid")
        if Humanoid and Humanoid.WalkSpeed ~= SpeedRunValue then
            Humanoid.WalkSpeed = SpeedRunValue
        end
    end
end)
